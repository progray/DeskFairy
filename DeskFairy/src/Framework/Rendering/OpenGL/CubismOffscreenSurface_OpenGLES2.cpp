...
void CubismOffscreenFrame_OpenGLES2::BeginDraw(GLint restoreFBO)
{
    ...
    if (restoreFBO < 0)
    {
        GLFuncs()->glGetIntegerv(GL_FRAMEBUFFER_BINDING, &_oldFBO);
    }
    else
    {
        _oldFBO = restoreFBO;
    ...
    GLFuncs()->glBindFramebuffer(GL_FRAMEBUFFER, _renderTexture);
}
...
void CubismOffscreenFrame_OpenGLES2::EndDraw()
{
    ...
    // 描画対象を戻す
    GLFuncs()->glBindFramebuffer(GL_FRAMEBUFFER, _oldFBO);
}

void CubismOffscreenFrame_OpenGLES2::Clear(float r, float g, float b, float a)
{
    // マスクをクリアする
    GLFuncs()->glClearColor(r,g,b,a);
...
csmBool CubismOffscreenFrame_OpenGLES2::CreateOffscreenFrame(csmUint32 displayBufferWidth, csmUint32 displayBufferHeight, GLuint colorBuffer)
{
    ...
    do
    {
        ...
        if (colorBuffer == 0)
        {
            ...
            GLFuncs()->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, displayBufferWidth, displayBufferHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
        ...
        GLFuncs()->glGetIntegerv(GL_FRAMEBUFFER_BINDING, &tmpFramebufferObject);

        GLFuncs()->glGenFramebuffers(1, &ret);
        GLFuncs()->glBindFramebuffer(GL_FRAMEBUFFER, ret);
        GLFuncs()->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, _colorBuffer, 0);
        GLFuncs()->glBindFramebuffer(GL_FRAMEBUFFER, tmpFramebufferObject);
...
void CubismOffscreenFrame_OpenGLES2::DestroyOffscreenFrame()
{
    if (_renderTexture!=0)
    {
        GLFuncs()->glDeleteFramebuffers(1, &_renderTexture);
        _renderTexture = 0;
    }
}
